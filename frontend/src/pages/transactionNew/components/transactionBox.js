              ...
              <tbody>
                {
                  context.cart.map((cart, index) => {
                    ...
                    <td>Rp. {numeral(cart.price).format('0,0.[00]')}</td>
                    <td>{cart.priceGroup === 'RETAIL' ? 'OUTLET' : cart.priceGroup}</td>
                    <td>{cart.details.unit}</td>
                    <td>{cart.qty}</td>
                    <td>Rp. {numeral(cart.price * cart.qty).format('0,0.[00]')}</td>
                  ...
                  <td className='font-bold'>
                    Rp. {
                      numeral(context.cart.map(cart => {
                        ...
                        return +subTotal
                      }).reduce((firstValue, secondValue) => firstValue + secondValue, 0)).format('0,0.[00]')
                    }
                  </td>
                </tr>
              </tbody>
