...
const Product = () => {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      ...
      let productData = []
      result.map(product => {
        const editButton = (<button className='btn bg-yellow-400 p-2 rounded' onClick={() => openModal(product)}>Atur Produk</button>)
  ...
  return (<React.Fragment>
    <div className='w-full'>
      <div className='w-full text-right'>
        <h1>Klik tombol ini untuk menambahkan produk : <button className='btn bg-green-400 p-2 rounded' onClick={() => openCreatorModal()}>+ Tambah Produk</button></h1>
      </div>
      <div className='w-full'>
        <DataTable
          search
