...
class Transaction extends Component {
        ...
        try{
            ...
            await productData.data.map(v => {
                if(v.status === "1"){
                    return this.setState({
                        productList: [
                            ...this.state.productList, {value: v.id, label: `[${v.category_details.name}] ${v.name} - Rp. ${numeral(v.price_per_unit_retail).format('0,0')} / ${v.unit}`}
                        ]
                    })
                }
        ...
        return this.state.cartItems.map((value, index) => {
            return (
                <tr className="bg-white hover:bg-gray-100" key={index}>
                    ...
                    <td className="text-center py-2 text-sm">{numeral(value.product.price_per_unit_retail).format('0,0')}</td>
                    ...
                    <td className="text-center py-2 text-sm">{numeral(value.quantity * value.product.price_per_unit_retail).format('0,0')}</td>
    ...
    removeItem = (val, index) => {
        ...
        return this.setState({
            productList: [
                ...this.state.productList, {value: val.id, label: `[${val.category_details.name}] ${val.name} - Rp. ${numeral(val.price_per_unit_retail).format('0,0')} / ${val.unit}`}
            ]
        })
    }
    ...
    saveTransaction = () => {
        ...
        this.state.cartItems.map(v => {
            ...
            return itemTable+=`<tr class="bg-white hover:bg-gray-100"><td class="text-center py-2 text-sm px-2">${v.product.name}</td><td class="text-center py-2 text-sm px-2">${numeral(v.product.price_per_unit_retail).format('0,0')}</td><td class="text-center py-2 text-sm px-2">${v.quantity}</td><td class="text-center py-2 text-sm px-2 text-right">${numeral(v.product.price_per_unit_retail * v.quantity).format('0,0')}</td></tr>\n`
        })
        ...
        return Swal.fire({
            ...
            <table class="table-auto w-full">
                ...
                <tr class="bg-gray-500">
                    ...
                    <td colspan=3 class="text-white text-sm text-right p-2">${numeral(grandTotal).format('0,0')}</td>
                </tr>
                </tbody>
            </table>`,
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                    ...
                                                    </div>
                                                    {
                                                        <div className="flex flex-col">
                                                            ...
                                                            Rp. {this.state.selectedProduct.product.price_per_unit_retail ? numeral(this.state.selectedProduct.product.price_per_unit_retail).format('0,0') : 0} /&nbsp;
