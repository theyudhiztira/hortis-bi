    ...
    loadTransactionData = async (from, to, limit, page) => {
        ...
        await data.map((value, index) => {
            return tableElement = [...tableElement, (<tr key={value.id}>
                ...
                <td className="text-sm p-2 font-normal text-center">{numeral(value.amount_due).format('0,0.[00]')}</td>
                        ...
                        <tbody>
                            {
                                trxDetails.items.map((item, index) => {
                                    return (<tr class="bg-white hover:bg-gray-100" key={index}>
                                        ...
                                        <td class="text-center py-2 text-sm px-2">{numeral(item.price).format('0,0.[00]')}</td>
                                        <td class="text-center py-2 text-sm px-2">{item.quantity}</td>
                                        <td class="text-center py-2 text-sm px-2 text-right">{numeral(item.price * item.quantity).format('0,0.[00]')}</td>
                                    </tr>)
                                })
                            }
                        <tr className="bg-gray-500">
                            <td colSpan="3" className="text-white text-sm text-right p-2">Grand Total</td>
                            <td colSpan="3" className="text-white text-sm text-right p-2">{numeral(trxDetails.amount_due).format('0,0.[00]')}</td>
                        </tr>
                    </tbody>
                    </table>
                </div>
